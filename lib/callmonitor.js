...
function CallMonitor(adapter, devices, phonebook) {
    ...
    function init() {
        ...
        client.on('close', _hadError => {
            adapter.log.debug('callmonitor closed ... reconnect');
        ...
        client.on('data', data => {
                ...
                for (const i of Object.keys(message)) {
                    if (i[0] !== '_') {
                        if (i === 'callee' || i === 'caller') {
                            dev.set(i, { val: message[i].toString(), common: { name: i, type: 'string', role: 'state' }});
                        } else {
                            dev.set(i, message[i]);
                        }
                    }
            ...
            switch (type) {
                case 'CALL':
                    message = {caller: array[4], callee: array[5], extension: parseInt(array[3]), timestamp, id};
                ...
                case 'CONNECT':
                    ...
                    message.extension = parseInt(array[3]);
                    set('connect');
                    break;
        ...
        adapter.log.debug('callmonitor initialize ...');
        client.connect({
