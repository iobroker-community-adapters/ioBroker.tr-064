...
const enableConnect1012 = '--- To use the callmonitor, enable connects to port 1012 on FritsBox by dialing #96*5* and restart this adapter';
...
module.exports = function (adapter, devices, phonebook) {
    ...
    client.on('data', function (data) {
        ...
        adapter.log.debug('callmonitor raw data: ' + JSON.stringify(raw));
        function set(name) {
            adapter.log.debug('callmonitor handle ' + name + ' message: ' + JSON.stringify(message));
            //const dev = new devices.CDevice(CALLMONITOR_NAME, '');
            //dev.force = true;
            if (timer) clearTimeout(timer);
            adapter.setState(CALLMONITOR_NAME + '.ringing', name === 'inbound', true);
            ...
            for (const i in message) {
                if (i[0] !== '_') adapter.setState(CALLMONITOR_NAME + '.' + name + '.' + i, message[i], true);
            }
            adapter.setState(CALLMONITOR_NAME + '.' + name + '.' + '.timestamp', timestamp, true);
            message._type = name;
            if (adapter.config.usePhonebook && phonebook) {
                ...
                adapter.setState(CALLMONITOR_NAME + '.' + name + '.' + '.callerName', message.callerName, true);
                ...
                adapter.setState(CALLMONITOR_NAME + '.' + name + '.' + '.calleeName', message.calleeName, true);
            }
            adapter.setState(CALLMONITOR_NAME + '.' + name + '.' + '.json', JSON.stringify(message), true);

            //dev.setChannel('', '');
            const pause = pauseStates[name];
            if (pause !== undefined) {
                adapter.setState(CALLMONITOR_NAME + '.toPauseState', pause, true);
                if (message.extension) adapter.setState(CALLMONITOR_NAME + '.toPauseState-' + message.extension, pause, true);
            }
