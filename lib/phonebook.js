...
Phonebook.prototype.read = function (bo, cb) {
    ...
    self.getPhonebookList(function (err, res) {
        ...
        function doIt() {
            ...
            adapter.log.debug('Try query Phonebook ' + phonebookId);
                ...
                require(u.protocol.slice(0,-1)).get({
                    ...
                    result.on('end', function () {
                        ...
                        Parser.parseString(data, function (err, json) {
                            ...
                            adapter.log.debug('Phonebook Data ' + phonebookId + ': ' + JSON.stringify(json) + ' / err=' + err);
                            for (let i = 0; i < json.phonebooks.phonebook.contact.length; i++) {
                                ...
                                for (let j = 0; j < numbers.length; j++) {
                                    ...
                                    adapter.log.debug('Register Phonebook Entry: ' + JSON.stringify(newEntry));
                                    self.push(newEntry);
                                }
                            }
...
Phonebook.prototype.start = function (sslDevice, cb, options) {
    ...
    this.init(sslDevice, function(res) {
        this.read(cb);
    }.bind(this), options);
};
