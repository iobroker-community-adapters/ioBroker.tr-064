...
adapter.log.debug('Calllist Refresh Inner 1');
    if (err || !data) return;
    adapter.log.debug('Calllist Refresh Inner 2');
    if (!systemData.native.callLists) return;
    adapter.log.debug('Calllist Refresh Inner 3');
    ...
    soef.getHttpData(url, {xml2json: true}, function (err, json) {
        ...
        systemData.save();
        adapter.log.debug('Calllist Refresh Inner Done');
        done && done();
    });
}
