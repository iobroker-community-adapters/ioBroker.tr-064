...
function refresh(err, data, cb, done) {
    ...
    adapter.log.debug('Request Callist JSON: url = ' + url);
    getHttpData(url, {xml2json: true}, (err, json) => {
        ...
        if (json && json.root) {
            ...
            if (Array.isArray(json.root.call) && json.root.call[0] && json.root.call[0].id && json.root.call[0].id < systemData.native.callLists.lastId) {
                systemData.native.callLists.lastId = 0;
                return refresh(err, data, cb, done);
            }
