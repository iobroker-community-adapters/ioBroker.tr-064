    ...
    adapter.on('stateChange', (id, state) => {
        if (state && id.startsWith(adapter.namespace + '.')) {
            if (initError) {
                ...
                return adapter.terminate ? adapter.terminate('tr-064 adapter not connected to a FritzBox. Terminating', 0) : setTimeout(() => process.exit(1), 2000);
            }
...
function main() {
    ...
    tr064Client.init(err => {
        ...
        if (err) {
            ...
            adapter.log.error('~');
            return adapter.terminate ? adapter.terminate('Fatal error. Can not connect to your FritzBox.', 0) : setTimeout(() => process.exit(1), 5000);
        }
